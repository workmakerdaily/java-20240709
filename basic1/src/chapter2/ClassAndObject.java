...
class TriangleMath {
	...
	// PI, getCircleArea(double)은 SRP(Single Responsibility Principle)(단일 책임 원칙)에 위배됨
	static final double PI = 3.1415;
	
	static double getCircleArea(double radius) {// 원의 넓이 구하기
	...
	static double getHypotenuse(double bottom, double height) { // 빗변 구하기
		
		if (bottom <= 0 || height <= 0) {
			return 0;
		}
		
		double hypotenuse2 = Math.pow(bottom, 2) + Math.pow(height, 2); //Math.pow(a, b); a에 b를 제곱한 값을 반환하는 함수
		double hypotenuse = Math.sqrt(hypotenuse2); //Math.sqrt(); 제곱근을 반환하는 함수
		
		return hypotenuse;
	}
	
	static double getCircumference(double bottom, double height, double hypotenuse) { // 둘레 구하기
	...
	static double getSin(double hypotenuse, double height) { // sin 구하기
